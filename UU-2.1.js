...
// 伪动态商品绑定数据(仅仅图片和价格)模块开始*******************************************************************************************************
var bindData = function(element, product){
    // http://icps.suning.com/icps-web/getVarnishAllPrice014/{cmmdtyCode}_{cityCode}_{area}_{bizCode}_{chan}_{callback}.vhtm
    // 获取当前城市cookie
    ...
    // 取区号
    var lesId = _snCityIdCookie('SN_CITY') && _snCityIdCookie('SN_CITY').split('_')[1] ? _snCityIdCookie('SN_CITY').split('_')[1] : '025';

    // 绑定从products.js读取的数据
    var $self = $(element);
    var proSrc = product.src,
        commodityCode = product.commodityCode,
        shopCode = product.shopCode;
        

    $self.find('.js-proSrc').attr('src', proSrc);

    if (!!commodityCode) {     
        commodityCode = formateCommodityCode(commodityCode);
        shopCode = formateShopCode(shopCode);

        // 动态获得价格数据并绑定
        var getPrice = function(){
            $.ajax({
                url: 'http://icps.suning.com/icps-web/getVarnishAllPrice014/' + commodityCode + '_' + lesId + '__' + shopCode + '_1_priceServiceCallBack.vhtm',
                dataType: 'jsonp',
                cache: true
            });           
        }();
        window.priceServiceCallBack = function(data){           
            var priceObj = data[0];
            var $element = $('[data-id='+priceObj.cmmdtyCode+priceObj.bizCode+']');
            var price = priceObj.price ? priceObj.price : '暂无报价';
            var refPrice = priceObj.refPrice ? priceObj.refPrice : (priceObj.snPrice ? priceObj.snPrice : priceObj.price);
            if (Number(refPrice) < Number(price)) {
                refPrice = '';
            };
            var downPayment = priceObj.bookPrice;
            $element.find('.js-promotionPrice').html(price);
            $element.find('.js-referencePrice').html(refPrice);
            $element.find('.js-downPayment').html(downPayment);
        }
...
// 伪动态商品绑定数据(仅仅图片和价格)模块结束*******************************************************************************************************

// 绑定从products.js读取的数据
UU.initProData = function(elements,productData){
    var validateData = function (data) {
        var isjson = typeof(data) == "object" && Object.prototype.toString.call(data).toLowerCase() == "[object array]" && data.length;
        return isjson;
    };
    var productDoms = $(elements);
    var productDomCount = productDoms.size();
    var productDataCount = productData.length;
    ...
    if (productDataCount < productDomCount) {
        var msg = '配置的产品数据少于展示的产品数据' + '（配置数据：' + productDataCount + '条；展示数据：' + productDomCount + '条；）';
        if (window.console) {
            console.log(msg);
        };
        
    }
    for (var i = 0; i < productDomCount; i++) {
        if (!!productData[i]) {
            var product = productData[i];
            var $self = productDoms.eq(i);
            var proSrc = product.src,
                brandSrc = product.brandSrc,
                QRCodeSrc = product.QRCodeSrc,
                ...
                commodityCode = product.commodityCode,
                shopCode = product.shopCode,
                couponNum = product.couponNum,
                proLink = product.link,
                flagshipLink = product.flagshipLink,
                couponLink = product.couponLink,
                downPayment = product.downPayment;
            
            if($self.hasClass('js-nowLoad')){
                bindData($self,product);            
            }else{
                $self.find('.js-proSrc').attr('alt', proName);
            };
            $self.find('.js-brandSrc').attr('src', brandSrc);
            $self.find('.js-proName').html(proName).attr('title', proName);
            $self.find('.js-proDiscount').html(proDiscount).attr('title', proDiscount);
            $self.find('.js-couponNum').html(couponNum);

            if (!commodityCode) {
                $self.find('.js-promotionPrice').html(promotionPrice);
                $self.find('.js-referencePrice').html(referencePrice);        
                $self.find('.js-downPayment').html(downPayment);        
            }else{            
                $self.attr( 'data-id', formateCommodityCode(commodityCode) + formateShopCode(shopCode) );               
            }
            // 添加爆炸贴代码
            if (!!$self.attr('data-flag')) {
                $self.prepend('<div class="'+$self.attr('data-flag')+'"></div>');
            }; 
            if (!!QRCodeSrc) {
                $self.append('<div class="QR-code"><img src="'+ QRCodeSrc +'"></div>');
            }else{
                $self.find('.js-proLink').attr('href', proLink);
                $self.find('.js-couponLink').attr('href', couponLink);
                $self.find('.js-flagshipLink').attr('href', flagshipLink);        
            }
        };
...
var formateCommodityCode = function(commodityCode){
    var prefix = '';
    if (commodityCode.length < 18) {
        for (var i = 0; i < 18 - commodityCode.length; i++) {
            prefix += '0';
        }
    }
    return prefix + commodityCode;        
};
var formateShopCode = function(shopCode){
    var prefix = '';
    if (shopCode.length < 10) {
        for (var i = 0; i < 10 - shopCode.length; i++) {
            prefix += '0';
        }
    }
    return prefix + shopCode;        
};